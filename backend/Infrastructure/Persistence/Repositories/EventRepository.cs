...
public class EventRepository : IEventRepository
{
    ...
    public async Task<IEnumerable<Event>> GetPublicEventsAsync(string? searchTerm = null, int page = 1, int pageSize = 12, List<string>? tags = null)
    {
        ...
        var whereClauses = new List<string> { @"e.""Visibility"" = 0" };

        if (tags != null && tags.Any())
        {
            sqlBuilder.Append(@"
                JOIN ""EventTags"" et ON e.""Id"" = et.""EventId""
                JOIN ""Tags"" t ON et.""TagId"" = t.""Id""
            ");
            whereClauses.Add(@"t.""Name"" = ANY(@Tags)");
            parameters.Add("Tags", tags);
        }

        if (!string.IsNullOrWhiteSpace(searchTerm))
        {
            whereClauses.Add(@"e.""Name"" ILIKE @SearchPattern");
        ...
        sqlBuilder.Append(@" ORDER BY e.""DateTime"" ASC LIMIT @PageSize OFFSET @Offset");
        parameters.Add("PageSize", pageSize);
        parameters.Add("Offset", (page - 1) * pageSize);

        using var connection = _connectionProvider.CreateConnection();
        return await connection.QueryAsync<Event>(sqlBuilder.ToString(), parameters);
    }

    public async Task<int> GetPublicEventsCountAsync(string? searchTerm = null, List<string>? tags = null)
    {
        var sqlBuilder = new StringBuilder(@"
