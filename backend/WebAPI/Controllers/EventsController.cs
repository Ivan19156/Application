...
public class EventsController : ControllerBase
{
    ...
    public async Task<IActionResult> GetPublicEvents(
        ...
        [FromQuery] int page = 1, 
        [FromQuery] int pageSize = 12,
        [FromQuery] List<string>? tags = null) // 👈 Додано параметр `tags`
    {
        // Передаємо `tags` у сервіс
        var events = await _eventService.GetPublicEventsAsync(search, page, pageSize, tags);
        return Ok(events);
    }
